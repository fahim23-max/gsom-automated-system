...
# --- MARKET SUMMARY (WEIGHTED AVERAGE CALCULATION) ---
def compute_summary(df):
    ...
    for category, group in temp_df.groupby("Securities Type"):
        count = int(group["ISIN"].count())
        total_crore = float(group["Outstanding_Crore"].sum())
        
        # Weighted Average Yield = Sum(Yield * Outstanding) / Sum(Outstanding)
        if total_crore > 0:
            weighted_yield = float((group["Yield_Val"] * group["Outstanding_Crore"]).sum() / total_crore)
        else:
            weighted_yield = 0.0
